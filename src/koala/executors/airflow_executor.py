...
class AirflowExecutor:
    ...
    def generate_airflow_dag(
        ...
        """Generate Airflow DAG file from Kola flow WITHOUT triggering execution.

        This is a convenience method for users who want to:
        - Preview generated DAG code
        - Deploy DAGs manually
        - Separate DAG generation from execution

        Args:
            flow: The DAGFlow to convert to Airflow DAG
            registry: Tool registry mapping names to functions
            dags_folder: Optional override for DAG file location (uses self.dags_folder if None)

        Returns:
            Path to the generated DAG file

        Raises:
            FlowError: If dags_folder not configured or generation fails

        Example:
            >>> executor = AirflowExecutor(dags_folder="./dags")
            >>> flow = dag("my-flow").step("double", "my_func", x=5).build()
            >>> registry = {name: meta.func for name, meta in default_registry._tools.items()}
            >>> dag_path = executor.generate_airflow_dag(flow, registry)
            >>> print(f"DAG generated at: {dag_path}")
        """
        target_folder = dags_folder or self.dags_folder

        if not target_folder:
            raise FlowError(
                "dags_folder must be specified either in constructor or as argument. "
                "Point it to your Airflow installation's dags directory."
            )

        logger.info(
            "generating_airflow_dag", flow_id=flow.id, target_folder=target_folder
        )
        ...
        try:
            ...
            file_path = os.path.join(target_folder, f"kola_{flow.id}.py")

            with open(file_path, "w", encoding="utf-8") as f:
                f.write(dag_code)

            logger.info("dag_generated", path=file_path, flow_id=flow.id)
            print(f"✅ DAG generated: {file_path}")
            print(f"📁 Flow ID: {flow.id}")
            print(f"📋 Steps: {len(flow.steps)}")
            print("\nNext steps:")
            print("1. Restart Airflow to load the DAG")
            print(f"2. Access UI at {self.airflow_url}")
            print(
                f"3. Trigger DAG '{flow.id}' with config: {{'question': 'your question'}}"
            )

            return file_path

        except Exception as e:
            logger.info("dag_generation_failed", error=str(e), flow_id=flow.id)
            raise FlowError(f"Failed to generate DAG for flow '{flow.id}': {e}") from e
    ...
    def _translate_to_airflow(
        ...
        tool_imports: Dict[str, list] = (
            {}
        )  # module_name -> list of (tool_name, func_name)
        tools_used = {step.action for step in flow.steps}
...
"""
        ...
        if tool_imports:
            ...
            for module_name, tools in sorted(tool_imports.items()):
                ...
                # Load environment if module has _load_env function
                dag_code += "    # Load environment variables if available\n"
                dag_code += f"    if hasattr({module_name}, '_load_env'):\n"
                dag_code += f"        {module_name}._load_env()\n"
                dag_code += (
                    f"        print(f'✅ Loaded environment from {module_name}')\n"
                )
...
"""
    ...
    def _write_dag_file(self, dag_id: str, code: str) -> None:
        ...
        """
        assert self.dags_folder is not None, "dags_folder must be set"
    ...
    def _trigger_dag(self, dag_id: str, conf: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            response.raise_for_status()
            return str(response.json()["dag_run_id"])

        except requests.RequestException as e:
            raise FlowError(f"Failed to trigger DAG '{dag_id}': {e}") from e
    ...
    def _fetch_results(self, dag_id: str, run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("failed_to_fetch_results", error=str(e))
            return {}
