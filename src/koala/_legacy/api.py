...
from .tools_legacy import default_registry
...
# Simple API-key based auth for the demo. Set env KOALA_API_KEYS to a
# comma-separated list of allowed keys. If unset, a default dev key
# ("dev-key") is allowed to avoid breaking local tests.
_API_KEYS: List[str] = [
    k.strip()
    for k in os.environ.get("KOALA_API_KEYS", "dev-key").split(",")
    if k.strip()
]
